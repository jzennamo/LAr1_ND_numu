...
//=========  (Mon Jan  6 15:48:30 2014) by ROOT version5.34/05
   ...
   TLatex *tex_pre = new TLatex(.12,.74,"PRELIMINARY");
   tex_pre->SetNDC();
   ...
   TLatex *tex_eff = new TLatex(.12,.79,"80% #nu#lower[0.4]{#mu} Efficiency");
   tex_eff->SetNDC();
   tex_eff->SetTextFont(62);
   tex_eff->SetTextSize(0.025);
   tex_eff->Draw();
   
   gROOT->ProcessLine(".x ./Sens_Matrix_LAr1ND_100m_T600_on_axis_Shape_and_Rate_5sigma_NoFlux.C(c3)");
   gROOT->ProcessLine(".x ./Sens_Matrix_LAr1ND_150m_T600_on_axis_Shape_and_Rate_5sigma_NoFlux.C(c3)");
   gROOT->ProcessLine(".x ./Sens_Matrix_LAr1ND_200m_T600_on_axis_Shape_and_Rate_5sigma_NoFlux.C(c3)");
